 ...
 * Send a property report email to a lead
 *
 * Body:
...
export async function POST(request) {
  try {
    ...
    await sendPropertyEmail({
      ...
      subject,
      html: emailHtml,
      from: account?.email || process.env.GMAIL_USER,
      agentName: account?.name || "Foster & Keys",
    });

    // Record the send in property_sends for each property
    await Promise.all(
      property_ids.map((pid) =>
        supabase.from("property_sends").insert({
          account_id: accountId,
          property_id: pid,
          lead_id,
          template_id: template.id,
          email_snapshot: emailHtml,
    ...
    console.error("POST /api/reports/send error:", err);
